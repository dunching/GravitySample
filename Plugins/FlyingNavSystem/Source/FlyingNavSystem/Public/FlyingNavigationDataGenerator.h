...
*/
class FLYINGNAVSYSTEM_API FRasteriseWorker final : public FNoncopyable, FGCObject
{
public:
...
protected:
	...
	*/
	void RasteriseLeafNode(FSVOLeafNode& Leaf, const FVector& LeafCentre, const FCoord SubNodeOffset, const FVector& SubNodeExtent);
	...
	*/
	static bool DoesVoxelOverlapWithRawGeometryData(
		const FSVORawGeometryElement& Geometry,
...
public:
	...
	virtual void Stop() override { RasteriseWorker->Cancel(); EnsureCompletion(); }
...
public:
	...
	void Cancel()
	{
		...
		// Stop and delete all WorkerTasks
	...
	uint32 GetAllocatedSize() const
	{
		...
		for (const TUniquePtr<FRasteriseWorkerTask>& WorkerTask : WorkerTasks)
		{
...
protected:
	...
	TArray<TUniquePtr<FRasteriseWorkerTask> > WorkerTasks;
