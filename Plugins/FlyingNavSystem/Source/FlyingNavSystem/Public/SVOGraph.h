...
// Used for storing 26 DOF directions, where each of X,Y,Z can be [-1,1]
struct FLYINGNAVSYSTEM_API FDirection
{
	...
	explicit FDirection(const int_fast8_t X = 0, const int_fast8_t Y = 0, const int_fast8_t Z = 0, const bool bConnected = false):
		X(X), Y(Y), Z(Z), bConnected(bConnected)
	{}
 ...
 */
struct FLYINGNAVSYSTEM_API FSVOPathfindingGraph : FGraphAStar<FSVOGraph, FGraphAStarDefaultPolicy, FGraphAStarDefaultNode<FSVOGraph> >
{
	typedef FSVOGraph::FNodeRef FGraphNodeRef;
	...
	*/
	bool ProcessSingleAStarNode(const FGraphNodeRef EndNodeRef, const bool bIsBound, const FSVOQuerySettings& Filter, int32& OutBestNodeIndex, FCoord& OutBestNodeCost);

	bool ProcessSingleThetaStarNode(const FGraphNodeRef EndNodeRef, const bool bIsBound, const FSVOQuerySettings& Filter, int32& OutBestNodeIndex, FCoord& OutBestNodeCost);

	bool ProcessSingleLazyThetaStarNode(const FGraphNodeRef EndNodeRef, const bool bIsBound, const FSVOQuerySettings& Filter, int32& OutBestNodeIndex, FCoord& OutBestNodeCost);
	...
	*/
	EGraphAStarResult FindSVOPath(const FGraphNodeRef StartNodeRef, const FGraphNodeRef EndNodeRef, const FSVOQuerySettings& QuerySettings, TArray<FGraphNodeRef>& OutPath);

	// Find a path from StartLocation to EndLocation through the Sparse Voxel Octree
	ENavigationQueryResult::Type FindPath(const FVector& StartLocation, const FVector& EndLocation, const FSVOQuerySettings& QuerySettings, TArray<FNavPathPoint>& PathPoints, bool& bPartialSolution);
