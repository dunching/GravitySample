...
void AFlyingNavigationData::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	...
	while (NumCalculatingPaths.GetValue() > 0)
	{
		ensure(AsyncTaskCompleteEvent->Wait());
	}
...
bool AFlyingNavigationData::ProjectPoint(const FVector& Point, FNavLocation& OutLocation, const FVector& ConnectedComponentPoint, FSharedConstNavQueryFilter Filter, const UObject* Querier) const
{
	...
	} else
	{
		...
		GetPathfindingGraph()->Graph.GetNeighbours(NodeLink, Neighbours);

		if (Neighbours.Num() > 0)
		{
			// Sort by lowest layer (ie prefer leaf nodes)
			Neighbours.Sort();
			OutLocation.NodeRef = Neighbours[0].AsNavNodeRef();
			OutLocation.Location = SVOData->GetPositionForLink(Neighbours[0]);
			return true;
		}*/

		// Check all 26 DOF
		TArray<FDirection> Directions;
		GetNeighbourGraph()->GetAvailableDirections(Point, ConnectedComponentPoint, Directions);
		if (Directions.Num() > 0)
		{
			// Prefer Z direction
...
ENavigationQueryResult::Type AFlyingNavigationData::CalcPathCost(const FVector& PathStart, const FVector& PathEnd, float& OutPathCost, FSharedConstNavQueryFilter Filter, const UObject* Querier) const
{
	// Cost = Length TODO: Modifiers
	return CalcPathLength(PathStart, PathEnd, OutPathCost, Filter, Querier);
}

ENavigationQueryResult::Type AFlyingNavigationData::CalcPathLength(const FVector& PathStart, const FVector& PathEnd, float& OutPathLength, FSharedConstNavQueryFilter QueryFilter, const UObject* Querier) const
{
...
ENavigationQueryResult::Type AFlyingNavigationData::CalcPathLengthAndCost(const FVector& PathStart, const FVector& PathEnd, float& OutPathLength, float& OutPathCost, FSharedConstNavQueryFilter QueryFilter, const UObject* Querier) const
{
	// Cost = Length TODO: Modifiers
...
FPathFindingResult AFlyingNavigationData::FindPath(const FNavAgentProperties& AgentProperties, const FPathFindingQuery& Query)
{
	...
	if (!IsInGameThread())
	{
		ON_SCOPE_EXIT
		{
			FlyingNavData->NumCalculatingPaths.Decrement();
		
			if (FlyingNavData->AsyncTaskCompleteEvent)
			{
				FlyingNavData->AsyncTaskCompleteEvent->Trigger();
			}
		};
	}
	...
	// Lock SVO Data
	FRWScopeLock DataLock(FlyingNavData->SVODataLock, SLT_ReadOnly);
	const FSVOData& SVOData = FlyingNavData->SVOData.Get();
  
	// Access Navigation Graph
	FSVOPathfindingGraph* NavigationGraph;
	...
	if (NavPath != nullptr)
	{
		...
		const APawn* Pawn = Controller ? Controller->GetPawn() : Cast<const APawn>(Owner);
		if (Pawn)
		{
		...
		const APawn* GoalPawn = NavPath->GoalPawn.Get();
		if (GoalPawn)
		{
		...
		if (Result.Result == ENavigationQueryResult::Success)
		{
			...
			// TODO: Movetemp on Main Thread?
			NavPath->GetPathPoints() = MoveTemp(PathPoints);
			//NavPath->PathPointsCriticalSection.Unlock();
			NavPath->MarkReady();
