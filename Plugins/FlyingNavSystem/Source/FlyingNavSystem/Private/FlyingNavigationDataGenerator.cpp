...
void FRasteriseWorker::PrepareGeometrySources()
{
	...
	for (const FBoxCenterAndExtent& Bounds : InclusionBounds)
	{
		NavigationOctree->FindElementsWithBoundsTest(Bounds, [&OwnerNavDataConfig, this](const FNavigationOctreeElement& Element)
		{
			...
			const UStaticMeshComponent* StaticMeshComp = Cast<const UStaticMeshComponent>(Element.GetOwner());
			if (StaticMeshComp)
			{
...
void FRasteriseWorker::GatherGeometryFromSources()
{
	...
	for (TSharedRef<FNavigationRelevantData, ESPMode::ThreadSafe>& ElementData : NavigationRelevantData)
	{
		...
		// The only thing that supports slicing atm is landscapes
		if (bSupportsSlices)
		{
...
void FRasteriseWorker::AddReferencedObjects(FReferenceCollector& Collector)
{
	for (const TSharedRef<FNavigationRelevantData, ESPMode::ThreadSafe>& RelevantData : NavigationRelevantData)
	{
...
FString FRasteriseWorker::GetReferencerName() const
{
	return FString::Printf(TEXT("FRasteriseWorker %d"), WorkerIdx);
}
...
void FSVOGenerator::RasteriseTick(const bool bBlockThread)
{
	...
	for (small_morton_t i = WorkerTasks.Num(); i < static_cast<small_morton_t>(NumThreads); i++)
	{
		...
		WorkerTasks.Add(
			MakeUnique<FRasteriseWorkerTask>(
				*this,
				GenerationBounds,
				i,
				Divisions
...
void FSVOGenerator::CollateRasterData()
{
	...
	// Wait for rasterise workers
	AllWorkersDispatchedEvent->Wait();
	
	// Copy over to NavData
	int32 NumLayerOneNodes = 0;
	for (int32 i = 0; i < WorkerTasks.Num(); i++)
	{
		WorkerTasks[i]->EnsureCompletion();
		NumLayerOneNodes += WorkerTasks[i]->GeneratedLayerOne().Num();
	}
	...
	for (int32 i = 0; i < NumThreads; i++)
	{
		...
		for (int32 j = 0; j < WorkerTasks[i]->GeneratedLayerOne().Num(); j++)
		{
			FSVONode& LayerOneNode = WorkerTasks[i]->GeneratedLayerOne()[j];
			if (LayerOneNode.bHasChildren)
			{
				...
				for (int32 k = 0; k < 8; k++)
				{
					FSVOLeafNode& LeafNode = WorkerTasks[i]->GeneratedLeafLayer()[FirstChildIndex + k];
		...
		LeafLayer.Append(WorkerTasks[i]->GeneratedLeafLayer());
		NumCopiedLeafNodes += WorkerTasks[i]->GeneratedLeafLayer().Num();
		
		LayerOne.Append(WorkerTasks[i]->GeneratedLayerOne());
		NumCopiedLayerOneNodes += WorkerTasks[i]->GeneratedLayerOne().Num();
	}
