...
public:
	FPathFindAction(UNavigationPath* NavPath,
		...
		AsyncQueryID(INVALID_NAVQUERYID)
	{
	...
	virtual ~FPathFindAction() override
	{
		...
		if (!Delegate.Get().bFinished && AsyncQueryID != INVALID_NAVQUERYID && Delegate->Path)
		{
			UNavigationSystemV1* NavSys = FNavigationSystem::GetCurrent<UNavigationSystemV1>(Delegate->Path);
			
			if (NavSys)
			{
				NavSys->AbortAsyncFindPathRequest(AsyncQueryID);
			}
		}
	}

	void SetQueryID(const uint32 QueryID) { AsyncQueryID = QueryID; }
...
void UCatmullRomSpline::FillLUT()
{
	...
	// Sanity check
	if (TotalSamples + 1 != DistanceLUT.Num())
	{
		printw("Reserved: %d, Added: %d", TotalSamples + 1, DistanceLUT.Num())
	}
}
...
uint32 UFlyingNavFunctionLibrary::FindPathAsync(
	...
	EPathFindingMode::Type Mode)
{
	...
	if (QueryID != INVALID_NAVQUERYID && FlyingNavData)
	{
	...
	const TSubclassOf<UNavigationQueryFilter> FilterClass)
{
	...
	if (NavSys != nullptr && NavSys->GetDefaultNavDataInstance() != nullptr)
	{
		...
		AFlyingNavigationData* FlyingNavData = Cast<AFlyingNavigationData>(NavigationData);
		...
		const uint32 QueryID = FindPathAsync(NavSys, FNavAgentProperties::DefaultProperties, Query, ResultDelegate, EPathFindingMode::Regular);
		if (QueryID != INVALID_NAVQUERYID)
		{
			PathFindAction->SetQueryID(QueryID);
...
bool UFlyingNavFunctionLibrary::IsPositionAValidEndpoint(const APawn* NavAgent, const FVector& Position, const bool bAllowBlocked)
{
	...
	FRWScopeLock ScopeLock(FlyingNavData->SVODataLock, SLT_ReadOnly);
...
UNavigationPath* UFlyingNavFunctionLibrary::SmoothPath(UObject* WorldContextObject, UNavigationPath* Path, const float SampleRate)
{
	...
	UCatmullRomSpline* CRSpline = MakeCatmullRomSpline(PathPoints);
	if (CRSpline != nullptr)
	{
		return SetNavigationPathPoints(WorldContextObject, Path, CRSpline->EquidistantSamples(SampleRate));
	}
